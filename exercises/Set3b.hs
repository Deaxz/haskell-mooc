...
kek n cur | cur <= n = sumTo cur : kek n (cur+1)
          | otherwise = []
...
sumsOf :: [Int] -> [Int]
sumsOf [] = []
sumsOf xs = sumPrevs [] xs

sumPrevs :: [Int] -> [Int] -> [Int]
sumPrevs prevs [] = []
sumPrevs prevs (x:xs) = sum prevs + x : sumPrevs (x:prevs) xs
